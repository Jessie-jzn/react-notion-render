...
export class NotionAPI {
  private readonly _apiBaseUrl: string; // API基础URL
  ...
  public async getSignedFileUrls(urls: SignedUrlRequest[]) {
  ...
  public async fetch<T>({
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          `Notion API error: ${error.response?.statusText || error.message}`
        );
      }
      throw error;
    }
